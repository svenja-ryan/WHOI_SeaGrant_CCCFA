...
'''
    ...
    14) change coordinates                   (changes 2D coordinates to 1D in orca output - only apply south of 30N)
    15) load_mesh                            (load mesh and mask files for specific region)
'''
'''
...
def anomaly(ax,x,y,offset):
    ...
    ax.fill_between(x, 0-offset[1], y, where = y<0-offset[1], facecolor='dodgerblue', interpolate=True)
#
#
#--------------------------------------------------------------------------
...
def mean_LC_width(data):
    mask = np.load('/home/sryan/Python/ORCA/NingalooNino/data/LC_mask_Furue.npz')
...
#
#----------------------------------------------------------------------------------
# change coordinates from x,y to lat,lon (1D) - only apply below 30N
def change_coordinates(data):
    # change to 1D coordinates
    temp = data.copy(deep=True)
    dummy = data.nav_lon[0,:].values
    dummy[dummy<0] = dummy[dummy<0]+360
    temp.coords['x'] = dummy
    temp.coords['y'] = data.nav_lat[:,0]
    temp = temp.rename({'x':'lon', 'y':'lat'}).set_coords(['lon','lat']).drop(['nav_lon','nav_lat'])
#     lon_bnds,lat_bnds = [30,210], [-40,30]
#     temp = temp.sel(lat=slice(*lat_bnds),lon=slice(*lon_bnds))
    return temp
...
#
#----------------------------------------------------------------------------------
# deaseason xarray, with option to provide reference period
def deseason(ds,timevar='time_counter',refperiod=None):
    dummy = timevar + '.month'
    if refperiod:
        if timevar=='time_counter':
...
def load_mesh(region,rename=False):
    ...
    elif region=='NWA': xbnds=slice(810,994); ybnds=slice(602,759)
